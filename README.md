...
For manual installation, download [the latest release](https://github.com/Kristijan1001/Jellyfin-VK-Videos/releases/latest), extract the ZIP into a `VK Videos_1.0.3.0` folder inside Jellyfin's `plugins` directory, and restart Jellyfin.
...
Playlists inside **VK Videos** now also open in clients that only open known library types; 1.0.2 covered only the tile itself. Jellyfin sends a channel's folders as their own item type, and the plugin now reports those as folder-style libraries too. Jellyfin Web is unaffected.
